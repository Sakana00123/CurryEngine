    ...
    void AnimationTimelineEditor::DrawTimelineArea()
    {
        ...
        if (m_selectedKey)
        {
            ...
            else
            {
				...
				// トラックの種類が変わった場合、マッピングをクリアして再構築
                {
                    static int prevSelectedTrackIndex = -1; // 前回選択されたトラックのインデックス
                    bool trackTypeChanged = false;
                    if (m_selectedKey)
                    {
                        if (prevSelectedTrackIndex != m_selectedKey->trackIndex)
                        {
                            prevSelectedTrackIndex = static_cast<int>(m_selectedKey->trackIndex);
                            assetIdToNameMap.clear();
                        }
                    }
                    else
                    {
                        prevSelectedTrackIndex = -1;
                    }
                }
				...
				case CurryEngine::Resources::AnimationEventType::ParticleEffect:
                {
                    ...
                    {
						ImGui::OpenPopup("ParticleEffectPopup");
					...
					if (ImGui::BeginPopup("ParticleEffectPopup"))
					{
