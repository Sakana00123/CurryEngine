...
void GeometricPrimitive::CreateCylinder(ID3D11Device* device, int segmentCount)
{
...
void GeometricPrimitive::CreateSphere(ID3D11Device* device, int stackCount, int sliceCount)
{
	std::vector<Vertex> vertices;
	for (int stack = 0; stack <= stackCount; ++stack) {
		float phi = DirectX::XM_PI * stack / stackCount;//緯度

		for (int slice = 0; slice <= sliceCount; ++slice) {
			float theta = DirectX::XM_2PI * slice / sliceCount;//経度
			//極座標から直交座標に変換
...
void GeometricPrimitive::CreateComBuffers(ID3D11Device* device, Vertex* vertices, size_t vertex_count,
